...
Column {
    ...
    function copyEntry(entry) {
        ...
        clipCopyProc.command = ["bash", "-c", pathPrepend + " ; cliphist decode <<< \"$1\" | wl-copy 2>/dev/null; true", "clip", entry]
        clipCopyProc.startDetached()
    }

    function deleteEntry(entry) {
        if (!/^\d+$/.test(entry)) return
        clipDeleteProc.command = ["bash", "-c", pathPrepend + " ; printf '%s' \"$1\" | cliphist delete 2>/dev/null; true", "clip", entry]
    ...
    ListView {
        ...
        delegate: Rectangle {
            ...
            radius: clipboardColumn.itemRadius
            color: "transparent"

            Rectangle {
                anchors.fill: parent
                anchors.leftMargin: 3 * root.uiScale
